...
from utils.modules import knn_point
...
class DGCNN_Grouper(nn.Module):
    def __init__(self, k_values=None):
        ...
        if len(k_values) != 4:
            raise ValueError(f"k_values must have exactly 4 elements, got {len(k_values)}")
        
        self.k_values = k_values
        print(f"DGCNN_Grouper initialized with k_values: {self.k_values}")
    ...
    def forward(self, x):
        ...
        f = self.get_graph_feature(coor, f, coor, f, k=self.k_values[0])
        f = self.layer1(f)
        f = f.max(dim=-1, keepdim=False)[0]

        coor_q, f_q = self.fps_downsample(coor, f, 512)
        f = self.get_graph_feature(coor_q, f_q, coor, f, k=self.k_values[1])
        f = self.layer2(f)
        f = f.max(dim=-1, keepdim=False)[0]
        coor = coor_q

        f = self.get_graph_feature(coor, f, coor, f, k=self.k_values[2])
        f = self.layer3(f)
        f = f.max(dim=-1, keepdim=False)[0]

        coor_q, f_q = self.fps_downsample(coor, f, 128)
        f = self.get_graph_feature(coor_q, f_q, coor, f, k=self.k_values[3])
        f = self.layer4(f)
        f = f.max(dim=-1, keepdim=False)[0]
        coor = coor_q
